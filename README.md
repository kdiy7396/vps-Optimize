...
```bash
# 标准安装（含 kernel-ml 升级）
bash -c "$(curl -fsSL https://raw.githubusercontent.com/kdiy7396/vps-Optimize/refs/heads/main/optimize.sh)"
```

```bash
# 跳过内核升级（仅网络调优）
SKIP_KERNEL=1 bash -c "$(curl -fsSL https://raw.githubusercontent.com/kdiy7396/vps-Optimize/refs/heads/main/optimize.sh)"
```

```bash
# 无人值守部署（自动重启）
AUTO_REBOOT=1 bash -c "$(curl -fsSL https://raw.githubusercontent.com/kdiy7396/vps-Optimize/refs/heads/main/optimize.sh)"
```
